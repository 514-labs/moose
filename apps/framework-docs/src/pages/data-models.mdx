...
<Python>
Data Models are represented as `dataclasses` (using the `@dataclass` decorator), and must be defined in a `.py` file. The `moose_data_model` decorator is used to register the dataclass with Moose:
...
<Callout type="info" title="Notes">
  ...
  - Any exported interface that adheres to the appropriate file and folder conventions will be automatically detected by Moose and used to generate the necessary infrastructure.
  - The dataclass must be passed as an argument to the `moose_data_model` decorator to register it with Moose. Other dataclasses in the file will be ignored.
</Callout>
...
moose_data_model(BasicDataModel)
```
</Python>
...
moose_data_model(DataModelWithOptionalField)
```
</Python>
...
moose_data_model(DataModelWithNestedObject) ## Only register the outer dataclass
```
