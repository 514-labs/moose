...
export default function Leaderboard({ leaderboard }: { leaderboard: LeaderboardEntry[] }) {
  ...
  const userLookup = {
    ...
    'Olivia': 'https://ca.slack-edge.com/T062WLJGPMW-U06ESHX61R7-466642c6a419-512',
      ...
      <ul className="space-y-4">
        {leaderboard.map((entry, index) => (
          <li key={index} className="flex items-center">
            <img 
              src={userLookup[entry.name as keyof typeof userLookup] || 'https://via.placeholder.com/40'} 
              alt={entry.name} 
              className="w-10 h-10 rounded-full mr-4"
            />
            <div className="flex-grow">
              <span className="font-semibold text-lg text-white">{entry.name}</span>
              <span className="block text-sm text-gray-400">{entry.value} {entry.metric == 'HEART RATE' ? 'BPM' : ''}</span>
            </div>
            <span className={`w-3 h-3 rounded-full ${index === 0 ? 'bg-green-500' : 'bg-gray-500'}`}></span>
          </li>
        ))}
      </ul>
    </div>
