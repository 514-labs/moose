...
After installation, close and reopen your terminal (or run `source ~/.bashrc` / `source ~/.zshrc`) to ensure the `moose` command is available in your PATH.
</Callout>
...
<Callout type="warning" title="Docker must be running" compact={true}>
Make sure Docker Desktop is running before proceeding. Moose uses Docker to run ClickHouse, Redpanda, and Temporal locally.
</Callout>
...
You should see an empty list or existing containers. If you get an error, start Docker Desktop and wait for it to fully initialize.
...
from .ingest.models import *      # Data models & pipelines
from .ingest.transform import *   # Transformation logic
from .apis.bar import *           # API endpoints  
from .views.bar_aggregated import * # Materialized views
from .workflows.generator import *  # Background workflows
```
</Python>

<Callout type="info" title="Why this matters" >

- The CLI scans the root file to find all your pipeline components
- If you create new models, APIs, or workflows but forget to export/import them, they won't be available
- This pattern keeps your code organized while ensuring everything is discoverable
</Callout>
...
```
<Callout type="info" title="Workflow Status" compact={true}>
The workflow runs in the background and is powered by [Temporal](https://temporal.io). You can check its status in the Temporal UI at `http://localhost:8080` (if you have the management dashboard enabled).
</Callout>

<ToggleBlock openText="(Bonus) Peek at your data" closeText="Hide details">
**What this command does:**
- Directly queries your ClickHouse database to show raw data
- Useful for debugging and verifying data was processed correctly
...
curl "http://localhost:4000/consumption/bar?limit=5&orderBy=totalRows"
```
