...
/// Converts a ClickhouseEngine to its string representation for serialization
pub fn engine_to_string(engine: &ClickhouseEngine) -> String {
    match engine {
...
/// Converts a string back to ClickhouseEngine for deserialization
pub fn engine_from_string(engine_str: &str) -> Option<ClickhouseEngine> {
    match engine_str {
        "MergeTree" => Some(ClickhouseEngine::MergeTree),
        "ReplacingMergeTree" => Some(ClickhouseEngine::ReplacingMergeTree),
        "AggregatingMergeTree" => Some(ClickhouseEngine::AggregatingMergeTree),
        "SummingMergeTree" => Some(ClickhouseEngine::SummingMergeTree),
        s if s.starts_with("S3Queue(") => {
            ...
            if let Some(content) = s.strip_prefix("S3Queue(").and_then(|s| s.strip_suffix(")")) {
                ...
                if parts.len() >= 2 {
                    ...
                    return Some(ClickhouseEngine::S3Queue {
