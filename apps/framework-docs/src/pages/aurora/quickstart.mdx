...
Aurora is a CLI and an Model Context Protocol (MCP) server which gives data engineering tools and context to LLM clients like Claude Desktop, Cursor, or Windsurf. These tools allow the clients  to interact with your data systems—for example, querying a Clickhouse database, inspecting a local Moose project, or generating new primitives like egress APIs.

This page includes quickstarts for three typical workflows:

- *Ad hoc analytics*: Connect your LLM client to a Clickhouse database and explore it using natural language. This uses the Clickhouse Playground as our example database, but you can use any Clickhouse database.

- *New analytics engineering project*: Scaffold a local Moose project from a remote Clickhouse schema and enrich it with metadata and new egress methods with Aurora. This uses the Clickhouse Playground as our example database, but you can use any Clickhouse database.

- *Template-based project*: Set up a Moose project with example ingest pipelines and test Aurora’s MCP tools on real data. This uses the ADS-B template as our example project, but you can use any Moose project template, or start a new project from scratch.
...
## Quickstart: New analytics engineering project from Clickhouse Playground or your own Clickhouse database
...
## Quickstart: New data engineering project from template
