...
This page covers OLAP migrations. For migrations across the Moose stack, see the Migrate docs.
</Callout>
...
- Development mode: You edit code, Moose infers the SQL and immediatelyapplies it to local ClickHouse. Great for rapid iteration; not guaranteed to infer intent (e.g., renames).
- Production (planned) mode: You generate a plan from the target environment vs your code, review and commit the plan, and Moose executes it deterministically during deploy with drift checks.

What you need to do:
- In dev: just code. Moose handles local diffs automatically.
- In prod (OLAP):
  ...
  - Deploy to production. Moose validates snapshots (current DB vs `remote_state.json`, desired code vs `local_infra_map.json`) and executes `plan.yaml` in order. If drift is detected, the deploy aborts; regenerate the plan and retry.
...
The plan is applied during deployment. Moose will validate that the remote database state matches the snapshot of the database state at the time the plan was generated, and applies `plan.yaml` in order; it aborts if snapshots don’t match current state.
...
- AddTable:
    ...
    deduplicate: false
- RawSQL:
    sql: "CREATE MATERIALIZED VIEW mv_to_target AS SELECT * FROM target_table"
```
