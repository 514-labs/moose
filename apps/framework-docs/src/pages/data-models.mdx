...
<Tabs.Tab>
In Typescript, Data Models are represented as `interfaces`, and must be exported from a `.ts` file (using `export` keyword):
...
<Tabs.Tab>
In Python, Data Models are represented as `dataclasses` (using the `@dataclass` decorator), and must be defined in a `.py` file:
