...
2. **Choose deployment platform** - Select from [Kubernetes](deploying-on-kubernetes), [ECS](deploying-on-ecs), [Docker Compose](deploying-with-docker-compose), or [offline deployment](deploying-on-an-offline-server)
3. **Configure for production** - Review [Production Mode](../runtime/prod) for comprehensive production configuration
