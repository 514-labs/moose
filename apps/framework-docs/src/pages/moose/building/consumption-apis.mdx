...
const exampleApi = new ConsumptionApi<QueryParams, ResponseBody[]>("example_endpoint",  
    async ({ filterField, maxResults }: QueryParams, { client, sql }) => {
        ...
        return await resultSet.json() as ResponseBody[];
    });
```
</TypeScript>
...
const myApi = new ConsumptionApi<QueryParams, ResponseBody>("my_api_route",
  async (
    ...
    const data = await client.query.execute<ResponseBody[]>(query);
...
api = ConsumptionApi[QueryParams, ResponseBody](name="my_api_route", query_function=run)
```
</Python>
...
const query = sql`
  ...
  FROM ${userTable.name}
  WHERE ${userTable.columns.isActive.name} = true
`;
