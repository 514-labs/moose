...
Generates migration scripts for models that are different from the previous version.
...
adds an entry of the current version and commit hash to `[supported_old_versions]` section in `project.toml`.
