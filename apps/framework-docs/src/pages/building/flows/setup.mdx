...
`.ts` file to represent this newly created data flow, in the following
directory structure:
...
my-moose-app/flows/UserActivity__ParsedActivity.ts
```
...
Now that you have your data flow file, you have to define the
...
Here's an example flow file that converts timestamps to UTC:
