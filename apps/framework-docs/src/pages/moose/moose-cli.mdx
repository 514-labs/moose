---
title: MooseStack CLI Reference
description: MooseStack CLI Reference
---

# MooseStack CLI Reference
...
### Init
Initializes a new MooseStack project.
```bash
...
### Build
Builds your MooseStack project.
```bash
...
### Prod
Starts MooseStack in production mode for cloud deployments.
```bash
moose prod
```
...
### Ps
View MooseStack processes.
```bash
moose ps
```

### Ls
View MooseStack primitives & infrastructure.
```bash
...
### Metrics
View live metrics from your MooseStack application.
```bash
moose metrics
```
...
```
- `--url`: Remote MooseStack instance URL (default: http://localhost:4000)
...
This reference reflects the current state of the MooseStack CLI based on the source code in the framework-cli directory. The commands are organized by their primary functions and include all available options and flags.