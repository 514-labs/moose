...
ORDER BY (`id`) "#;
    ...
    #[test]
    fn test_create_table_query_collapsing_merge_tree() {
        let table = ClickHouseTable {
            version: Some(Version::from_string("1".to_string())),
            name: "test_table".to_string(),
            columns: vec![
...
ORDER BY (`id`) "#;
    ...
    #[test]
    fn test_create_table_query_collapsing_merge_tree_error() {
        let table = ClickHouseTable {
            version: Some(Version::from_string("1".to_string())),
            name: "test_table".to_string(),
            columns: vec![ClickHouseColumn {
        ...
        assert!(matches!(
            result,
            Err(ClickhouseError::InvalidParameters { message }) if message == "CollapsingMergeTree requires an order by clause"
        ));
    }

    #[test]
    fn test_create_table_query_versioned_collapsing_merge_tree() {
        let table = ClickHouseTable {
            version: Some(Version::from_string("1".to_string())),
            name: "test_table".to_string(),
            columns: vec![
            ...
            order_by: vec!["id".to_string(), "version".to_string()],
...
ORDER BY (`id`,`version`) "#;
    ...
    #[test]
    fn test_create_table_query_versioned_collapsing_merge_tree_error() {
        let table = ClickHouseTable {
            version: Some(Version::from_string("1".to_string())),
            name: "test_table".to_string(),
            columns: vec![ClickHouseColumn {
        ...
        assert!(matches!(
            result,
            Err(ClickhouseError::InvalidParameters { message }) if message == "VersionedCollapsingMergeTree requires an order by clause"
        ));
    }

    #[test]
    fn test_engine_try_from_collapsing_merge_tree() {
        let engine = ClickhouseEngine::try_from("CollapsingMergeTree").unwrap();
        assert!(
            matches!(engine, ClickhouseEngine::CollapsingMergeTree(sign_col) if sign_col == "sign")
        );
    }

    #[test]
    fn test_engine_try_from_versioned_collapsing_merge_tree() {
        let engine = ClickhouseEngine::try_from("VersionedCollapsingMergeTree").unwrap();
        assert!(matches!(
            engine,
            ClickhouseEngine::VersionedCollapsingMergeTree { sign_column, version_column }
            if sign_column == "sign" && version_column == "version"
        ));
    }

    #[test]
    fn test_engine_try_from_shared_collapsing_merge_tree() {
        let engine = ClickhouseEngine::try_from("SharedCollapsingMergeTree").unwrap();
        assert!(
            matches!(engine, ClickhouseEngine::CollapsingMergeTree(sign_col) if sign_col == "sign")
        );
    }
    ...
    #[test]
    fn test_engine_try_from_shared_versioned_collapsing_merge_tree() {
        let engine = ClickhouseEngine::try_from("SharedVersionedCollapsingMergeTree").unwrap();
        assert!(matches!(
            engine,
            ClickhouseEngine::VersionedCollapsingMergeTree { sign_column, version_column }
            if sign_column == "sign" && version_column == "version"
