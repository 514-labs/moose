...
Control how Moose manages your table resources when your code changes.
See the [LifeCycle Management guide](./lifecycle) for detailed information.

<TypeScript>
```ts filename="LifeCycleTable.ts" copy
import { OlapTable, LifeCycle } from "@514labs/moose-lib";

// Production table with deletion protection
const prodTable = new OlapTable<UserData>("prod_users", {
  orderByFields: ["id", "timestamp"],
  deduplicate: true,
  lifeCycle: LifeCycle.DELETION_PROTECTED
});

// Development table with full management
const devTable = new OlapTable<UserData>("dev_users", {
  orderByFields: ["id"],
...
<Python>
```py filename="LifeCycleTable.py" copy
from moose_lib import OlapTable, OlapConfig, LifeCycle

# Production table with deletion protection
prod_table = OlapTable[UserData]("prod_users", OlapConfig(
...
# Development table with full management
dev_table = OlapTable[UserData]("dev_users", OlapConfig(
    order_by_fields=["id"],
