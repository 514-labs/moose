...
description: Consumption APIs for Moose
---
...
## Overview
Query APIs are functions that run on your server and automaticallly exposed as HTTP `GET` endpoints. They are designed to read data from your OLAP database.

Query APIs automatically provde the the database client to your function, and handles connection pooling/lifecycle management for you. These special type of APIs also provide automatic type validation and type conversion for your query parameters, which are sent in the URL, and response body.
...
Please refer to the [OpenAPI](/stack/open-api) page for more information on how to integrate your APIs with OpenAPI.
