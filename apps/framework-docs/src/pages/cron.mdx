...
Cron jobs in Moose provide a mechanism to schedule and execute arbitrary scripts or make HTTP `GET` calls at specified intervals. These jobs are defined within the `moose.project.toml` configuration file. Moose cron jobs are useful for automating recurring tasks, for example:
>>>>>>> cea168b8 (docs & landing)
...
## Configuration
Cron jobs are defined in the `[cron_jobs]` array within the configuration file. Each job entry should include:
>>>>>>> cea168b8 (docs & landing)
