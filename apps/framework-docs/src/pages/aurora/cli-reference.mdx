...
Connects to an existing data source.

```
aurora connect <data-source> <--connection-string <connection-string>> <--mcp <claude-desktop>>
```

- `<data-source>`: The data source to connect to. Currently only `clickhouse` is supported.
