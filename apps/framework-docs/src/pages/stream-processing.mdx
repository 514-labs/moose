...
They are defined as regular functions in TypeScript or Python files, and are executed in real-time as data flows into your Moose application.
...
- `SourceDataModel__DestinationDataModel.ts` (for TypeScript)
- `SourceDataModel__DestinationDataModel.py` (for Python)
...
<Tabs.Tab>
In TypeScript, a Streaming Function is defined as the default export of its module:
...
<Tabs.Tab>
In Python, a Streaming Function is defined as a `Flow` object with a `run` attribute that is assigned the function:
...
- The `Flow` object is how Moose identifies the entry point for the Streaming Function.
...
<Callout type="info" title="What About the Function Name?">
  The function name itself is not important, as Moose identifies the function by
  whatever is assigned to the `run` attribute of the Flow object.
</Callout>
