...
description: Create and configure OLAP tables in Moose
---
...
const table = new OlapTable<Record>("table", {
  orderByFields: ["id"],
  engine: ClickHouseEngines.ReplacingMergeTree  // Use ReplacingMergeTree for deduplication
});
```
</TypeScript>
...
If you have a table that is managed by an external system (e.g Change Data Capture like ClickPipes), you can still use Moose to query it. You can set the config in the table config to set the lifecycle to `externally-managed`.
...
<BulletPointsCard
  title="Table Creation Best Practices"
  bullets={[
    {
      title: "Use IngestPipeline for end-to-end flows",
      description: "Create tables as part of ingestion pipelines for automatic API and stream integration"
    },
    {
      title: "Use standalone tables for custom workflows",
      description: "Create standalone tables when you need fine-grained control or external data sources"
    },
    {
      title: "Design indexes for your query patterns",
      description: "Configure orderByFields based on your most common query patterns"
    },
    {
      title: "Use appropriate ClickHouse types",
      description: "Leverage LowCardinality, NamedTuple, and Decimal types for performance optimization"
    },
    {
      title: "Plan for data lifecycle",
      description: "Use appropriate lifecycle management for development vs production tables"
    }
  ]}
/>

<Callout type="info">
For more advanced table configuration and lifecycle management, see the [OLAP Tables documentation](/moose/building/olap-table) and [Data Modeling documentation](/moose/building/data-modeling).
</Callout>
