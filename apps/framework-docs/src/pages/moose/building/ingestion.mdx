...
Moose supports several ingestion patterns to accommodate different data sources and requirements:

### Direct OlapTable Insertion

For programmatic data insertion with full validation and error handling, use the OlapTable `insert()` method:

<TypeScript>
```ts filename="DirectInsert.ts" copy
import { OlapTable } from "@514labs/moose-lib";

const table = new OlapTable<MyDataModel>("my_table");
...
<Callout type="info">
For detailed information about direct insertion, validation methods, and error handling strategies, see the [OlapTable documentation](/moose/building/olap-table#direct-data-insertion).
</Callout>
