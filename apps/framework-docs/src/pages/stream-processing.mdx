...
They are defined as regular functions in <TypeScript>TypeScript</TypeScript><Python>Python</Python> files, and are executed in real-time as data flows into your Moose application.
...
<Python>
A Streaming Function is defined as a `Flow` object with a `run` attribute that is assigned the function:
