...
export default function App() {
  const [selectedMetric, setSelectedMetric] = useState<MetricType>('HEART RATE')
  const metrics: MetricType[] = ['HEART RATE', 'HEART RATE ZONES', 'POWER', 'CALORIES']
  ...
  const fetchData = async () => {
    try {
      ...
      const response = await fetch(`http://localhost:4000/consumption/getHR?timestamp=${timestamp}`) 
  ...
  useEffect(() => {
    ...
    if (latestDataPoint) {
      const newLeaderboard: LeaderboardEntry[] = Object.entries(latestDataPoint)
        .filter(([username, value]) => typeof value === 'number' && username !== 'timestamp')
        .map(([username, value]) => ({
          name: username,
  ...
  useEffect(() => {
    const fetchCoaching = async () => {
      try {
        ...
        const response = await fetch(`http://localhost:4000/consumption/getCoaching`);
  ...
  return (
    <div className="iphone-frame bg-black text-white flex flex-col min-w-[380px] rounded-[40px] shadow-xl">
          ...
          {metrics.map((metric) => (
            <button
              ...
              className={`px-4 py-2 rounded-full font-semibold ${
                selectedMetric === metric ? 'bg-gray-800 text-white' : 'text-gray-400'
              }`}
