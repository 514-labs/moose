...
1. Inside a Moose Project:
   - Use the version specified in the project's package.json or requirements.txt
...
2. Outside a Moose Project:
   ...
   - If no version is installed, automatically fetch and use the latest available version
   - New projects will be initialized with this latest version
...
# Install specific Moose version
moose install 1.0.0

# Install latest Moose version
moose install latest

# Update Moose project version
moose update latest  # Updates project config files
```
...
# Get absolute path for MCP
moose --binary-path  # Returns absolute path to current version
```
...
3. Essential Commands:
   ```bash
   moose install <version>  # Install specific version
   moose install latest    # Install latest from stable
   moose install latest --channel dev  # Install latest from dev
   moose install @branch-name --channel dev  # Install from git branch
   ```
...
5. Minimal Project Integration:
   ...
   - Channel specification in project files
...
2. Additional Commands:
   ```bash
   moose cleanup           # Basic cleanup
   moose versions         # List installed versions
   moose current         # Show current version
   moose channel         # Show/switch channels
   ```
...
2. New Commands:
   ```bash
   moose cleanup --keep N  # Keep N versions
   moose cleanup --days N  # Keep N days of versions
   moose versions --stats  # Show usage statistics
   moose channel list     # List available channels
   ```
...
3. CI/CD Integration:
   ...
   - Channel pinning for deployments
