...
MooseStack automatically manages database migrations by comparing your code with your database schema. Migrations are applied automatically when:
...
<Callout type="info" title="Migration Safety" icon={ShieldCheck} compact>
In development, migrations are applied immediately. In production, migrations are applied during deployment with additional safety checks.
</Callout>
...
When you deploy your code to production, MooseStack follows this migration process:

1. **Schema Comparison**: MooseStack compares the final code in your repository with the state of your production database schema
...
class ChangedType(BaseModel):
    id: Key[str]
    some_column: int  # Changed type from string to int
```
</Python>
...
When you modify a materialized view's SELECT statement, MooseStack will:
