...
Amazon ECR is straightforward and is designed to work out of the box with ECS. Using Docker Hub works just fine if your moose container is publicly available; however, if your container is private, you'll need to do a bit more work to provide ECS with your Docker credentials.
...
In the next section below, we'll see instructions for configuring your container environment variables.
...
> - For the Redpanda message timeout delay consider using a value of greater than one 1000ms (1 second) if you're using a hosted Redpanda cloud service.
