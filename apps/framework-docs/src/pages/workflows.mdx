...
import { Python, TypeScript, Callout } from "../components";
...
<Python>
```bash
moose-cli workflow init example_workflow --steps firstStep,secondStep
```
</Python>
...
Workflows can receive input parameters that are passed to their first step. These parameters can be provided through the CLI or via `MooseClient.workflows`.

### CLI Example

```bash
moose-cli workflow run hello_world --input_value="test"
```
...
- Write small, focused steps that do one thing well.
- Log important information at each step.
- Use meaningful names for workflows and steps.
- Handle errors gracefully with appropriate recovery logic.
...
| **Problem** | **Solution** |
| --- | --- |
| Workflow times out | Check timeout in config.toml and step performance |
| Step missing data | Verify previous step's return value matches expected input type |

## Next Steps

- Learn more about [Consumption APIs](/consumption-apis)
- Explore [Moose CLI](/moose-cli)
