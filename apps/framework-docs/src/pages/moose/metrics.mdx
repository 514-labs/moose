---
title: Observability
description: Unified observability for MooseStack across development and production—metrics console, health checks, Prometheus, OpenTelemetry, logging, and error tracking
---
...
This page consolidates MooseStack observability for both local development and production environments.

## Local Development

### Metrics Console

MooseStack provides a console to view live metrics from your MooseStack application. To launch the console, run:
...
MooseStack applications expose a health check endpoint at `/health` that returns a 200 OK response when the application is operational. This endpoint is used by container orchestration systems like Kubernetes to determine the health of your application.

In production environments, we recommend configuring three types of probes:

1. Startup Probe: Gives MooseStack time to initialize before receiving traffic
...
MooseStack applications expose metrics in Prometheus format at the `/metrics` endpoint. These metrics include:

- HTTP request latency histograms for each endpoint
- Request counts and error rates
- System metrics for the MooseStack process
...
In production deployments, MooseStack can export telemetry data using OpenTelemetry. Enable via environment variables:
...
Join our Slack community to share feedback and get help with MooseStack.
</Callout>
