...
import { Callout, LanguageSwitcher, TypeScript, Python, TypeScriptInline, PythonInline } from "../../components";

# Implementing Streaming Functions

<LanguageSwitcher />

Streaming Functions are just ordinary <TypeScriptInline>Typescript</TypeScriptInline><PythonInline>Python</PythonInline> functions. Inside the function, you can leverage the full power of the language to process, enrich, filter, or otherwise manipulate the source data as needed.
...
export default function manipulateData(source: SourceDataModel): ManipulatedDestinationDataModel {
  ...
  const manipulatedNumber = source.int_field1 + source.int_field2;
  const manipulatedString = source.string_field.toUpperCase();
  const manipulatedDate = new Date(source.timestamp_field).toISOString();
...
def manipulate_data(source: SourceDataModel) -> ManipulatedDestinationDataModel:
    # Manipulate multiple fields from the source data into the destination format.
    manipulated_integer = source.field1 + source.field2
    manipulated_string = source.field3.upper()
    manipulated_date = source.field4.isoformat()

    return ManipulatedDestinationDataModel(
        id=source.id,
        manipulated_integer=manipulated_integer,
        manipulated_string=manipulated_string,
        manipulated_date=manipulated_date,
    )
...
By returning <TypeScriptInline>`null`</TypeScriptInline><PythonInline>`None`</PythonInline> you can discard the data.
...
You can use the <TypeScriptInline>`fetch`</TypeScriptInline><PythonInline>`requests`</PythonInline> module to enrich your source data with additional data from an external API.
