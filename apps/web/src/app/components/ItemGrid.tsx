...
const getDefaultStyle = (className: string, cols: number, rows: number, itemPosition: ItemPosition) => {
    
    if (className) {
        return className + " " + baseSizeStyles(itemPosition) + " " + composeSizes(cols, rows) ;
    } else {
        return baseSizeStyles(itemPosition) + " " + composeSizes(cols, rows) ;
    }
}

export const ItemGrid = ({children, cols, rows, className, ref, itemPosition}: GridProps) => {
    
    return (
        <div className={getDefaultStyle(className, cols, rows, itemPosition)} ref={ref}>
            {children}
        </div>
    )
}