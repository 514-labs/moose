...
In the `moose-deployment.yaml` file, illustrated in the following section, you'll see a block that describes the health check endpoint.
...
This will allow Kubernetes to determine the health of your moose service.

## Configuring container environment variables

    Inside your `moose-deployment.yaml` file you'll need to add an `env` section for environment variables. In the following example we're using Kubernetes secretes which is an industry best practice.
...
We've created a handy shell script that you can use in your cloud shell to create the above secrets.
