...
## Creating a Materialized View with `createAggregation()`
...
<Callout type="info" emoji="💡">
  Materialized Views are pre-computed queries stored as their own tables. When
  new data is added to the source table, the database updates the materialized
  view with the aggregated data.
</Callout>
...
| `orderBy` | Columns that define the primary key for the target table where your query results will be stored. To define multiple columns, put them in parentheses e.g. `orderBy: “(column1, column2)”` | Yes      |
