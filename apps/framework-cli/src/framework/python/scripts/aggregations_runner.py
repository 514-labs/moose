...
def get_agg_from_file(path):
    ...
    agg_objs = [obj for obj in dir(agg) if isinstance(getattr(agg, obj), agg_def)]
...
print(get_agg_from_file(files[0]))
...
#     try:
#         ch_client.execute(f"DROP VIEW IF EXISTS {file_name}")
