...
function writeFile(fileName: string, document: BrainwaveData): void {
  ...
  s.write(
    `${document.bandOn}\t${document.acc.x}\t${document.acc.y}\t${document.acc.z}\t${document.gyro.x}\t${document.gyro.y}\t${document.gyro.z}\t${document.alpha}\t${document.beta}\t${document.delta}\t${document.gamma}\t${document.theta}\t${document.ppm?.channel1 ?? ""}\t${document.ppm?.channel2 ?? ""}\t${document.ppm?.channel3 ?? ""}\r\n`,
  );
  screen.render();
}
