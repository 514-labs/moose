...
<Callout type="info" title="OLAP Migrations" href={`${PathConfig.olap.path}/apply-migrations`} ctaLabel="Immediate Apply">
To apply changes immediately, use the OLAP migrations guide.
</Callout>

<Callout type="info" title="Planned DDL (OLAP only)" href={`${PathConfig.olap.path}/planned-migrations`} ctaLabel="Planned Flow">
Generate, review, and execute a planned OLAP DDL migration via `plan.yaml` with drift checks.
</Callout>
