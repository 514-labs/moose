...
## Configuring container environment variables
    ...
    Inside your `moose-deployment.yaml` file you'll need to add an `env` section for environment variables. In the following example we're using Kubernetes secrets which is an industry best practice.
