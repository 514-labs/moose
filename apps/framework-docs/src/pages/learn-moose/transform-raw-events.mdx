...
<Callout type="info">
  ...
  source data model into a target destination model.
</Callout>
...
You can see from the above that most of the fields map to fields in the RawStarEvents Data Model, but we’re missing language data. We will fetch the languages information from the GitHub API within the Streaming Function. Let's walk through how to set this up!
...
<Callout type="info" title="Breaking Down the Code">
  ...
  **`callGitHubAPI()`** are helper functions to interact with the GitHub API.
  We're simply writing modular TypeScript code here—neat, right? 😊 - The
