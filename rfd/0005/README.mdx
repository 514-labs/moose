...
- Maintains existing behavior for Sloan CLI installation
...
```bash
# Sloan - direct installation (unchanged behavior)
bash -i <(curl -sSL https://www.fiveonefour.com/install.sh) sloan
...
bash -i <(curl -sSL https://www.fiveonefour.com/install.sh) sloan,moose

# Installing a specific version
bash -i <(curl -sSL https://www.fiveonefour.com/install.sh) moose@1.0.0,sloan@1.0.0
...
1. Detect which CLI is being installed (Sloan or Moose)
2. For Sloan:
...
```bash
# Install Sloan (unchanged behavior)
curl -sSL https://www.fiveonefour.com/install.sh | sh -s sloan
...
5. **Unified Version Management for Both CLIs**
   - Apply same version management to both Sloan and Moose
   - Pros: Consistent behavior across tools
   - Cons: Unnecessary complexity for Sloan, breaks existing workflows
...
- No impact on Sloan performance (maintains direct installation)
...
- Installation script: Dual-mode support for Sloan and Moose
...
- Clear distinction between Sloan and Moose installation patterns
...
- No impact on Sloan users (maintains existing behavior)
...
1. Basic Wrapper Script:
   ...
   - Support `moose` vs `sloan` installation modes
