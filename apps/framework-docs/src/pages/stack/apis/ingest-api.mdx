...
Control how Moose manages your pipeline resources when your code changes.
See the [LifeCycle Management guide](./lifecycle) for detailed information.

<TypeScript>
```ts filename="LifeCyclePipeline.ts" copy
import { IngestPipeline, LifeCycle, Key } from "@514labs/moose-lib";
...
// pipeline with deletion protection
const events = new IngestPipeline<UserEvent>("events", {
  table: true,
  stream: true,
  ingest: true,
...
<Python>
```py filename="LifeCyclePipeline.py" copy
from moose_lib import IngestPipeline, IngestPipelineConfig, Key, LifeCycle
from pydantic import BaseModel
...
# pipeline with deletion protection
prod_pipeline = IngestPipeline[UserEvent]("events", IngestPipelineConfig(
    table=True,
    stream=True,
    ingest=True,
    # automatically applied to the table and stream
