...
After ingesting the data, you might have noticed the CLI message: `Received UserActivity_0_0 -> ParsedActivity_0_0 1 message(s).` This indicates that Moose successfully parsed the data from the UserActivity model to the ParsedActivity model, using the transformation function defined in the `UserActivity__ParsedActivity.ts` file in the `/functions/` folder.

To learn more about how Moose's Streaming functions primitive works and how to define and execute custom transformation functions, check out the functions documentation.
