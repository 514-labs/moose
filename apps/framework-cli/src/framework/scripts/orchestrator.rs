...
"#,
    ...
    #[tokio::test]
    async fn test_workflow_running() -> Result<(), Status> {
        let (_temp_dir, workflow_dir, workflow_id) =
            create_unique_workflow().map_err(|e| Status::internal(e.to_string()))?;

        tokio::spawn(async move {
            execute_workflow(SupportedLanguages::Python, &workflow_dir, None)
                .await
                .unwrap();
        });
        ...
        let mut found = false;
        for _i in 0..10 {
            println!("Checking if workflow is running");
            tokio::time::sleep(tokio::time::Duration::from_millis(500)).await;
            match is_workflow_running(&workflow_id).await {
                Ok(true) => {
                    found = true;
                    break;
                }
        ...
        assert!(found, "Workflow should have been found and running");

        // Clean up by stopping the workflow
        stop_workflow(&workflow_id).await?;
