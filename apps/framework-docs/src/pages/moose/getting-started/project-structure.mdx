...
```typescript
// app/index.ts
export { UserEvents } from './ingest/events';
export { UserDataMaterializedView } from './views/user-analytics';
export { UserAnalyticsAPI } from './apis/user-analytics';
```
</TypeScript>
...
| models         | Define your [Data Models](/moose/building/data-modeling) that serve as schemas for all components |
| ingest         | Create [IngestPipeline](/moose/building/ingestion) objects that combine ingest APIs, streams, and tables |
...
<TypeScript>
```ts filename="app/models/records.ts" copy
...
<Python>
```py filename="app/models/records.py" copy
...
import { RawRecord, ProcessedRecord } from '../models/records';
...
from app.models.records import RawRecord, ProcessedRecord
...
RawRecords.stream.add_transform(ProcessedRecords.stream, transform_record)
```
</Python>
