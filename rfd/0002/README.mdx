...
## Technologies we dismissed

Some of the technologies we initially looked at and dismissed

### OLAP

- Snowflake: Doesn't run localy
- BiQuery: Doesn't run locally
- Pinot: Looked really complicated and heavy
- Druid: Looked really complicated and heavy
- DuckDB: Not suitable for production load with concurrency
- Custom: Too costly
- Delta Lake: Only can append to tables, needs Spark to do processing
- Iceberg:
- Hudi:
- TiDB: Hard to operationalize, fairly heavy to build and run locally

### Streaming Engines

- Kafka: Too heavy to operationalize
- NATS: Protocole less popular than Kafka
