...
This feature is currently in beta. We're working on complete type support for all ClickHouse data types. Tables containing unsupported types are currently skipped during the initialization process. Check the [Current Limitations](#current-limitations) section below to see what's currently supported.
</Callout>
...
<Python>
```bash copy
moose init moose-clickhouse-playground --from-remote https://explorer:@play.clickhouse.com:443/?database=default --language python
```
</Python>

<TypeScript>
```bash copy
moose init moose-clickhouse-playground --from-remote https://explorer:@play.clickhouse.com:443/?database=default --language typescript
```
</TypeScript>
</Callout>
...
- **Partial support**:
  - `LowCardinality(T)` → maps to base type `T` (full support coming soon)
  - `JSON` types → `any` (Python) / `Record<string, any>` (TypeScript) (better typing coming soon)
