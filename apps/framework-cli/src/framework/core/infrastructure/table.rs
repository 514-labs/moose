...
use crate::infrastructure::olap::clickhouse::queries::{
    engine_from_string, engine_to_string, ClickhouseEngine,
};
...
impl Table {
    ...
    pub fn expanded_display(&self) -> String {
        format!(
            ...
            self.engine
                .as_ref()
                .map(|e| format!(" - engine: {}", engine_to_string(e)))
                .unwrap_or_default()
        )
    }
    ...
    pub fn from_proto(proto: ProtoTable) -> Self {
        Table {
            ...
            engine: proto
                .engine
                .into_option()
                .and_then(|wrapper| engine_from_string(&wrapper.value))
                .or_else(|| {
                    proto
                        .deduplicate
