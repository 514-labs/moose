...
const exampleApi = new ConsumptionApi<QueryParams, ResultItem[]>("example_endpoint",  
    ...
    { version: "1" }, // Optional version config
  );
```
...
# Create the API
example_api = ConsumptionApi[QueryParams, ResponseBody](name="example_endpoint", query_function=run, version="1") # Version config is optional
```
<ToggleBlock openText="Show Source Pipeline" closeText="Hide Source Pipeline">
