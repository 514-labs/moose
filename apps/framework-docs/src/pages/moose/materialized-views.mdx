...
Materialized views are a powerful feature of Moose that allow you to precompute and store the results of complex queries. This can be useful for improving query performance, reducing the load on your database, and providing a more efficient way to access data.

## Overview

Materialized views in Moose:
- **Precompute complex queries**: Store results of expensive operations for faster access
- **Automatically update**: Refresh when source tables change
- **Support aggregations**: Efficiently calculate metrics across your data
- **Type-safe definitions**: Leverage TypeScript for schema validation
