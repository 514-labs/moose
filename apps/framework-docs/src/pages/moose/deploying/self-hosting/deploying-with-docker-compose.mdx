...
Here is a docker compose file that will deploy the entire Moose stack:
...
services:
  ...
  # Your Moose application
  moose:
    image: :latest  # Update with your image name
    depends_on:
...
1. After starting Clickhouse with the initial admin user, connect to create additional users:
...
3. For additional security in production, consider using Docker secrets for passwords.
...
After starting the services, you can create a namespace for your Moose application:

```bash
# Create an alias for the tctl command
alias tctl="docker exec temporal tctl"
