...
Unlike the `run()` function, the `on_cancel` callback does not receive any input arguments.
</Python>
...
Unlike the `run()` function, the `onCancel` callback does not receive any input arguments.
</TypeScript>
