...
<Tabs.Tab>
  ...
  <Tabs.Tab>
    ...
    ```
    <BulletPointsCard
      title="One codebase for everything. Like web DX, but for analytics"
      bulletStyle="check"
      compact={true}
      bullets={[
        "Zero glue code",
...
<Card className="space-y-8 my-8">
  ...
  <StaggeredCard className="border-t">
    ...
    <StaggeredCode>
      ```ts
      const etl = new EtlPipeline("etl", {
        extract: async (context) => {
          return await fetch("some-source-url").json();
        },
...
<Card className="space-y-8 my-8 p-4">

### Zero-Config Local Dev Environment
