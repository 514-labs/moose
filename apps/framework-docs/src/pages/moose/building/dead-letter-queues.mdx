...
const exampleDLQ = new DeadLetterQueue<ExampleSchema>("exampleDLQ");

const pipeline = new IngestPipeline<ExampleSchema>("example", {
  ingest: true,
  stream: true,
  table: true,
  deadLetterQueue: exampleDLQ, // Route failed ingestions to DLQ
});
```
</TypeScript>
...
pipeline = IngestPipeline[ExampleSchema](
    ...
    config=IngestPipelineConfig(
        ...
        stream=True,
        table=True,
        dead_letter_queue=example_dlq  # Route failed ingestions to DLQ
    )
)
```
</Python>
