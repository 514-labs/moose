...
Materialized views are write-time pipelines. They run a static `SELECT` to transform data from one or more source table(s) to a target destination table. You query the destination table like any other table.
...
from moose_lib import MaterializedView, OlapTable, ClickHouseEngines, sql
from source_table import source_table

# Define the schema of the transformed rows-- this is static and it must match the results of your SELECT. It also represents the schema of your entire destination table.
class TargetSchema(TypedDict):
    id: str
...
mv = MaterializedView[TargetSchema](MaterializedViewOptions(
    ...
    """,
    select_tables=[source_table],
    target_table=OlapConfig(name="serving_table", order_by_fields=["id"]),
...
mv = MaterializedView[TargetSchema](MaterializedViewOptions(
    ...
    """,
    select_tables=[source_table],
    target_table=target_table, ## This is the standalone table that is managed independently from the MV
    materialized_view_name="mv_to_target_table",
))
```
</Python>
...
mv = MaterializedView[Dest](MaterializedViewOptions(
    ...
    """,
    select_tables=[source_table],
    target_table=OlapConfig(name="proj_table"),
...
# Create the destination table explicitly
daily = OlapTable[DailyCounts](OlapConfig(
    name="daily_counts",
    engine=ClickHouseEngines.SummingMergeTree,
    order_by_fields=["day", "user_id"],
))
...
mv = MaterializedView[Dest](MaterializedViewOptions(
    ...
    target_table=OlapConfig(name="user_activity_by_day"),
