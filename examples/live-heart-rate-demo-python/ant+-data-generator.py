    ...
    while True:
        for device_id in device_ids:
            ...
            if device_data[device_id]['hr_event'] or device_data[device_id]['packet_number'] == 0:
                # True implies at least one heart rate event has occurred
                # Need to generate a new heart rate for this device - if there is no history, use a random value for a starting point
                device_data[device_id]['target_hr'] = generate_realistic_heart_rate(time_elapsed,base_hr=device_data[device_id]['rhr'],max_hr=device_data[device_id]['hr_max'], phase=device_data[device_id]['phase'])
                device_data[device_id]['hr_event'] = False
            
            # Generate the packet and get the heart rate event status
            ant_packet = generate_ant_hrm_packet(
                device_id, 
                device_data[device_id]
            )
            
            # Convert the packet to a JSON-formatted string
            json_data = json.dumps(ant_packet)
            #print(json_data)
            try:
                # Send the packet via HTTP POST
        ...
        # Wait for 0.25 seconds before sending the next packet
        time.sleep(0.25)
