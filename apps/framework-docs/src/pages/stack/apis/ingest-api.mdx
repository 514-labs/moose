...
## Validation
Moose validates all incoming data against your <TypeScript inline>interface</TypeScript> or <Python inline>Pydantic model</Python>. If a record fails validation, Moose can automatically route it to a Dead Letter Queue (DLQ) for later inspection and recovery.
