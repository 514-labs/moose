...
When you need to stream data into your ClickHouse tables, you can set the `Stream.destination` as a reference to the `OlapTable` you want to insert into. This will automatically provision a synchronization process that intelligently batches and inserts data into the table.
...
cosnt etlTask = new Task<null, void>({
    name: "ETL",
    run: async () => {
...
### Via REST API
You can leverage the built in MooseAPI module to place a `POST` REST API endpoint in front of your streams and tables to allow you to insert data from external applications.
...
<Callout type="info" title="OpenAPI Client Integration" >
With these APIs you can leverage the built-in OpenAPI client integration to gemnerate API clients in your own language to connect to your pipelines from external applications.
</Callout>
