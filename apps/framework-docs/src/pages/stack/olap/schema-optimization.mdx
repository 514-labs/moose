...
Nullable columns in ClickHouse have significant performance overhead. Use default values instead when possible.
...
import { OlapTable, NamedTuple } from "@514labs/moose-lib";
...
class UserEvent(BaseModel):
    ...
    user_id: str
    event_type: str
    location: NamedTuple[float, float, str, str]  # lat, lon, city, country
    metadata: NamedTuple[str, str, int]           # version, source, priority
    created_at: Date

# ❌ Bad: Using nested objects (less efficient)
class BadUserEvent(BaseModel):
    id: str
    location: dict  # Nested - less efficient
...
### `orderByFields` should prioritize LowCardinality first
  ...
  bullets={[
    {
      ...
      description: "Use default values instead of nullable types for better performance"
    },
    {
      title: "Use LowCardinality for limited values",
      description: "Most efficient string type for columns with few unique values"
    },
    {
      title: "Choose smallest integer type",
      description: "Pick UInt8/Int16/etc. based on your data range to save storage"
    },
    {
      title: "Use appropriate DateTime precision",
      description: "Balance storage vs precision needs (seconds vs milliseconds)"
    },
    {
      title: "Use Decimal for financial data",
      description: "Avoid floating-point precision issues in calculations"
    },
    {
      title: "Prefer NamedTuple over Nested",
      description: "More efficient for structured data in ClickHouse"
    },
    {
      title: "Optimize orderByFields",
      description: "Prioritize columns used in WHERE/GROUP BY and put LowCardinality first"
    }
  ]}
/>
