import { ToggleBlock, Callout } from "../../components";
import { Tabs } from "nextra/components";

# Ingest Sample Data

Test out the `/ingest/RawStarEvent` API endpoint on some sample data with this `curl` command:

<ToggleBlock openText="Expand to view full command" closeText="Hide command" open>    
...
### Query the `RawStarEvent` Table

<Tabs items={['Using the SQLTools VSCode Extension', 'Using Another DB Explorer']}>
