...
function EmptyListItem({ itemName }) {
  return (
    <div className="hover:bg-accent hover:text-accent-foreground hover:cursor-pointer">
      <div className="py-4 text-muted-foreground">No {itemName} to show yet</div>
      <Separator />
    </div>
  )
}
...
export default async function OverviewPage(): Promise<JSX.Element> {
  ...
  return (
    <section className="p-4 grow">
      ...
      <div className="mb-20">
        ...
        <div className="grid grid-cols-3 gap-4">
          <div className="col-span-3 xl:col-span-1">
            ...
            <Separator />
            {data.models && data.models.length > 0 ? data.models.slice(0, 10).map((model, index) => (
          ...
          <div className="col-span-3 xl:col-span-1 flex flex-col">
            <OverviewCardHeader title="Flows" numItems={0} href="https://docs.moosejs.com" />
            <Separator />
            <EmptyListItem itemName="flows" />
          </div>
          <div className="col-span-3 xl:col-span-1 flex flex-col">
            <OverviewCardHeader title="Insights" numItems={0} href="https://docs.moosejs.com" />
            <Separator />
      ...
      <div>
        <div className="text-3xl py-6 text-muted-foreground">Infrastructure</div>
        <div className="grid grid-cols-3 gap-4">
          <div className="col-span-3 xl:col-span-1">
            <OverviewCardHeader
            ...
            <Separator />
            {data.ingestionPoints && data.ingestionPoints.length > 0 ? data.ingestionPoints.slice(0, 10).map((ingestionPoint, index) => (
          ...
          <div className="col-span-3 xl:col-span-1">
            <OverviewCardHeader title="Tables" numItems={data.tables.filter(t => t.engine !== "MaterializedView" && !t.name.includes(".inner")).length} 
            href="infrastructure/databases/tables?type=table" />
          
            <Separator />
            {data.tables && data.tables.length > 0 ? data.tables.filter(t => t.engine !== "MaterializedView" && !t.name.includes(".inner")).slice(0, 10).map((table, index) => (
          ...
          <div className="col-span-3 xl:col-span-1">
            ...
            <Separator />
            {data.tables && data.tables.length > 0 ? data.tables.filter(t => t.engine === "MaterializedView").slice(0, 10).map((table, index) => (
              <Link href={`/infrastructure/databases/${table.database}/tables/${table.uuid}`} key={index} >
                <div className="hover:bg-accent hover:text-accent-foreground hover:cursor-pointer">
                  <div className="py-4 text-muted-foreground">{table.name}</div>
                  <Separator />
