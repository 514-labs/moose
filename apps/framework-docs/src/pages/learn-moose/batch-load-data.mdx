...
Create a new file for the ingest script:
...
class StargazerIngester:
    ...
    def ingest_stargazers(self, stargazers: List[Dict]) -> int:
        """Ingest stargazers into the HistoricalStargazer table via the API endpoint"""
        ingested_count = 0
...
2. Processes each stargazer and sends their data to your Moose application
3. Includes error handling and progress reporting
...
2. Send each stargazer record to your Moose application's `HistoricalStargazer` ingestion endpoint
...
```
(1) Ingested armanjindal - starred at 2024-07-30T01:04:06Z
(2) Ingested okane16 - starred at 2024-09-25T13:40:24Z
...
