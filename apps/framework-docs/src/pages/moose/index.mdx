...
export const examplePipeline = new IngestPipeline<ExampleModel>("example", {
  ingest: true, // Creates HTTP POST endpoint with validation
  stream: true, // Creates Redpanda streaming topic
  table: true, // Creates ClickHouse analytical database table
});
```
</Tabs.Tab>
<Tabs.Tab>
...
example_pipeline = IngestPipeline("example", IngestPipelineConfig(
    ingest=True, ## Creates HTTP POST endpoint with validation
    stream=True, ## Creates Redpanda streaming topic
    table=True, ## Creates ClickHouse analytical database table
))
...
No more writing boilerplate or managing multiple moving parts. Just define your data models and transformations, and Moose will automatically generate the infrastructure to support them, as well as all the glue code to wire it all together.


By default, Moose comes pre-configured with the following open source technologies:
- [ClickHouse](https://github.com/ClickHouse/ClickHouse) (realtime analytical database)
- [Redpanda](https://github.com/redpanda-data/redpanda) (streaming) 
...
## Why Moose Exists
<ZoomImg light="backend-light.png" dark="backend-dark.png" alt="Why Moose Exists" />
