...
<Tabs items={["Aurora CLI", "MCP.json"]}>
  <Tabs.Tab>
    ...
    aurora config tools
    ```
    and enable `read-only-moose-tools`. e.g.:
...
**Tools used:** `write_data_model`, `write_workflow`, `write_stream_function`, `write_materialized_view`
...
**Tools used:** `write_spec`, `write_data_model`, `write_workflow`, `write_stream_function`, `write_materialized_view`, `create_egress_api`
...
<Tabs items={["Aurora CLI", "MCP.json"]}>
  <Tabs.Tab>
    ...
    aurora config tools
    ```
    and enable `remote-clickhouse`. e.g.:
    ```txt filename="Terminal" copy
    ? Select experimental tools to enable (no selection defaults to read-only and egress-tools):  
      [x] read-only - Enable read-only tools for data inspection
      [ ] egress-tools - Enable egress-tools for exposing data (requires read only tools: auto enables)
      [ ] experimental-moose-tools - Enable experimental moose tools (requires read only tools: auto enables)
      [x] remote-clickhouse - Enable Remote Clickhouse integration
      [ ] external-duck-db-mcp - Enable External DuckDB MCP integration
    ```
  </Tabs.Tab>
  <Tabs.Tab>
