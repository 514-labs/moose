...
With the `TopProgrammingLanguages` Materialzied View created in the previous step, you can build a [Consumption API](/consumption-apis) to fetch data from this view and make it available for your other applications.
...
- Dynamically generate and execute a query to extract analyzed data from the `TopProgrammingLanguages` materialized view
...
<Python>
```py filename="/app/apis/ranked_languages.py" copy {9-37}
from moose_lib import MooseClient
from typing import Optional

def run(client: MooseClient, params, jwt_payload: Optional[dict] = None):
...
This function will return a ranked list of programming languages based on the <TypeScript NodeType="span">`rankBy`</TypeScript><Python NodeType="span">`rank_by`</Python> parameter. If an invalid value is passed in for <TypeScript NodeType="span">`rankBy`</TypeScript><Python NodeType="span">`rank_by`</Python>, the function will throw an error. If no value is passed in for <TypeScript NodeType="span">`rankBy`</TypeScript><Python NodeType="span">`rank_by`</Python>, the function will use the default value of <TypeScript NodeType="span">`total_projects`</TypeScript><Python NodeType="span">`total_projects`</Python>.
