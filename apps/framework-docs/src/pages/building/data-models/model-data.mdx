...
### Using the VSCode SQLTools Extension (recommended)

When you open your project in VSCode, Moose automatically helps you install and configure the [SQLTools](https://marketplace.visualstudio.com/items?itemName=mtxr.sqltools) and [SQLTools ClickHouse Driver](https://marketplace.visualstudio.com/items?itemName=ultram4rine.sqltools-clickhouse-driver) extensions to connect to the Clickhouse database that is running on your local development server.

### Configuring a Database Explorer (manual)

You can use any database explorer to connect to the Clickhouse database running on your local development server. You can find the connection details in the `project.toml` file located in the root of your project. The connection details are under the `[clickhouse_config]` section.
